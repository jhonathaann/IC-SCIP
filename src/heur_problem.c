...
void printLPvars(SCIP* scip, SCIP_VAR** pvars, int n1, int nfrac, int n0)
{
   ...
   printf("\nTotal de vars com valor 1.0 = %d\n", n1);
   for(i=0;i<n1;i++){
      solval = SCIPgetVarSol(scip,pvars[i]);
      printf("%s (%lf)\n", SCIPvarGetName(pvars[i]), solval);
   }
   printf("\nTotal de vars com valor frac = %d\n", nfrac);
   for(i=n1;i<n1+nfrac;i++){
      solval = SCIPgetVarSol(scip,pvars[i]);
      printf("%s (%lf)\n", SCIPvarGetName(pvars[i]), solval);
   }
   printf("\nTotal de vars com valor 0.0 = %d\n", n0);
#ifdef DEBUG
 ...
 * @param custo the current cost of the solution, that must be updated
 * @return always return 1 (not used yet). 
 */
int updateSolution(SCIP_VAR* var, instanceT *I, int* covered, int *nCovered, int *custo)
{
   int a, i;
...
SCIP_RETCODE selectCand(SCIP* scip, SCIP_VAR** solution, int nInSolution, int custo, SCIP_VAR** pvar, SCIP_VAR** varlist, int n1, int nfrac, int* covered)
{
// only select actived var in scip_cp and whose rounding up is valid for the problem
   ...
   found = 0;
   bestSol = NULL;
   // ignoring variables with values 1.0 since they were already included into the solution
   /*
   for (c = 0; c < n1 && !found; c++)
   {
      var = varlist[c];
